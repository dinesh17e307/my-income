...
class AddListMilk extends Component {
    state = {
        kg: '',
        rate: '',
        date: '',
    ...
    sendListToDB = () => {
        ...
        set(ref(db, `milk/${this.state.userName}/` + this.state.date), data);
        this.setState({
            kg: '',
            rate: '',
            date: '',
    ...
    goToShowList = () => {
        this.props.history.push(`/showlistmilk?user=${this.state.userName}`)
    }
    render() {
            ...
            <Grid item style={{ margin: '50px' }} className={classes.outerContainer}>
                ...
                {!this.state.loading && (<Grid container >
                    <Grid item lg={6} xs={12} >
                        ...
                        }} variant='standard' label="Litre" id="kg" placeHolder="Litre" onChange={(event) => this.onChangehandler(event)} value={this.state.kg} />
                    </Grid>
                    <Grid item lg={6} xs={12}>
...
export default withStyles(AddListStyle)(AddListMilk)